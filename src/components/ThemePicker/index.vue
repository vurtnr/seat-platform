...
export default class extends Vue {
  ...
  private async onThemeChange(value: string) {
    ...
    if (!this.chalk) {
      const url = `http://unpkg.com/element-ui@${version}/lib/theme-chalk/index.css`
    ...
    let styles: Array<HTMLElement> = [].slice.call(document.querySelectorAll('style'))
    styles = styles
      .filter(style => {
        const text = style.innerText
        return new RegExp(oldValue, 'i').test(text) && !/Chlak Variables/.test(text)
      })
  ...
  private updateStyle(style: string, oldCluster: Array<string>, newCluster: Array<string>) {
    let newStyle = style
